...
public class IDE {
	...
	public void Play(MouseEvent mouseEvent) {
		...
		if(selectedTab != null) {
			...
			Utilities.RunCommand(projectFolderPath + "\\compile-script.bat");
			// Call JUnit with the class file
			Utilities.RunCommand(projectFolderPath + "\\run-script.bat");
		}
		else {
			// Get the "console" TextArea
			TextArea taConsole = (TextArea) ideStage.getScene().lookup("#console-text-area");
	...
	public static void AddTab(String name) {
		...
		if (name.contains("🗋 ")) {
			...
			if (alreadyOpen == false) {
				...
				Document doc = Main.loadProjectSettingsFile(IDE.projectFolderPath + "\\ProjectSettings.xml");
	...
	public static org.w3c.dom.Node getFileNodeForCurrentTab(Tab tab, Document doc) {
		...
		if (files.getLength() > 0) {
			...
			for (int i = 0; i < files.getLength(); i++) {
				...
				String name = Main.getXmlNodeAttribute(files.item(i), "name");
